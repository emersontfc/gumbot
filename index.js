...
async function startXeonBotInc() {
    ...
    // Get Profile Picture Group
       ...
       XeonBotInc.send5ButImg(pea[0].id, `「As configurações do grupo foram alteradas 」\n\nAs informações do grupo foram restritas, agora apenas o administrador pode editar as informações do grupo!`, `Configurações do grupo alterar a mensageme`, wm_fatih, [])
       ...
       XeonBotInc.send5ButImg(pea[0].id, `「Configurações do grupo alteradas 」\n\nAssunto do grupo foi alterado para *${pea[0].subject}*`, `Mensagem de alteração das configurações do grupo`, wm_fatih, [])
     }
    })
...
🐶✑ Bem-vindo ao nosso confortável feliz😋, às vezes barulhento😜, geralmente bagunçado🤥, cheio de amor  , CASA😌!!`} )
                } else if (anu.action == 'remove') {
                    XeonBotInc.sendMessage(anu.id, { image: Tol, contextInfo: { mentionedJid: [num] }, caption: `🐶✑ @${num.split("@")[0]} Left ${metadata.subject}

🐶✑ Não tenho certeza se foi obrigado a nos dar essa despedida, mas foi divertido enquanto durou 😌✨` })
                }
            }
        } catch (err) {
            console.log(err)
